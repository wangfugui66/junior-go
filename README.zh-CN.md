...
- **Codex 角色配置：** `.codex/config.toml` 限制 loop 并发，`.codex/agents/*.toml` 为六个角色分别设置模型、推理强度、输出简洁度和 sandbox 默认值。
- **共享 loop 资产：** `agents/`、`skills/`、`workflows/`、`memory/` 仍然是同一套源文件，不按平台 fork
  出两份。

Codex 只会在你将该仓库标记为“已信任”后加载项目内的 `.codex/` 配置。

这套 Codex 默认策略把较深推理留给专门找方案漏洞的 `adversarial-architect`：它使用 `gpt-5.6-sol` + `high`；调研、计划和实现使用 `gpt-5.6-terra` + `medium`；真跑验证和 junior 讲解使用 `gpt-5.6-luna` + `low`。所有角色都要求低输出冗长度，`.codex/config.toml` 还把并发子 agent 限为 2、嵌套深度限为 1。Codex 当前没有受支持的“按角色硬 token 预算”字段，所以这里写的是能真实生效的节流手段，不伪造一个看似严格却不会执行的 token 上限。
